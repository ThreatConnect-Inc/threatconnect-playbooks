...
2. Click **Import Playbook → Import Playbook** (top right).
   ...
   ![Import Playbook menu](images/01_import_playbook_menu.png)

3. Select `(BETA) Active Intel Requirement Brief - Daily.pbxz` to upload
4. After import, verify:
   - Name reads **Daily IR Findings Brief (BETA)**
   - Version reads **1.0**

**Quirk worth knowing:** If the customer instance already has a previous copy of this playbook with an overlapping name, the import may show stale data or fail silently. Clear any older copies from the Playbooks list before testing.

---

## Step 2 — Configure required scope variables

Open the **⚙️ SETTINGS - CHANGE ME** step at the start of the playbook canvas. Two variables are the bare minimum for any deployment:
...
2. Click the play button under the green **Timer Trigger**
...
5. Open the email and skim the cards. You should see clickable titles and clickable links back to ThreatConnect, as well as a short write-up
...
**If the run fails or times out:** see **Troubleshooting** below.
